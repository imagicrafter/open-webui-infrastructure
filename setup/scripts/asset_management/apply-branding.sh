...
#   - host: Save to host directory (auto-detects Phase 1 vs Phase 2)
#
# Phase Detection:
#   - Phase 2: Saves to /opt/openwebui/{client}/static/ (volume-mounted, persists automatically)
#   - Phase 1: Saves to /opt/openwebui/{client}/branding/ (requires post-startup injection)
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    if [ $# -lt 2 ] || [ $# -gt 3 ]; then
        ...
        echo
        echo "  host      - Save to host directory for persistent branding (RECOMMENDED)"
        echo "              Auto-detects Phase 1 or Phase 2 deployment:"
        echo "              - Phase 2: Saves to static/ (volume-mounted, persists automatically)"
        echo "              - Phase 1: Saves to branding/ (requires post-startup injection)"
